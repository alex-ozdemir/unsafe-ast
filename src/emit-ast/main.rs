...
fn emit_unsafe_ast<'a,'tcx,'ast>(crate_name: &str,
                                  crate_type: String,
                                  krate: &hir::Crate,
                                  session: &'ast Session,
    ...
    if crate_name != "build_script_build" {
        let mut v = unsafe_ast::UnsafeASTEmitter::new(tcx, session);
        krate.visit_all_items(&mut v);
        errln!("{{\"name\": {:?}, \"type\":{:?}, \"functions\":{} }}",
...
impl<'a> AnalyzeUnsafe<'a> {
    ...
    pub fn unsafe_ast_emitter() -> AnalyzeUnsafe<'a> {
        AnalyzeUnsafe::new(Box::new(move |state| {
            ...
            emit_unsafe_ast(crate_name, crate_type, krate, session, tcx);
        }))
    }
}
